...
impl Fixture {
    ...
    pub fn new() -> Self {
        ...
        f.write_cmd("claude", "commands", "postgres-patterns", "PG patterns", "body pg");
        f.write_cmd("claude", "agents", "code-reviewer", "Reviews code", "Agent doc.\n\nSee ~/.claude/commands/caveman.md");

        // opencode
        f.write_cmd("opencode", "commands", "api-design", "API design", "body api");
        f.write_skill("opencode", "caveman", "Caveman skill", "skill body");
        f.write_skill("opencode", "api-design", "API design skill", "skill body");
        f.write_cmd("opencode", "agents", "frontend-developer", "FE dev", "Agent.\n\nSee ~/.opencode/skills/caveman/SKILL.md");
    ...
    fn write_cmd(&self, cli: &str, category: &str, name: &str, desc: &str, body: &str) {
        let dir = self.tmp.path().join(cli).join(format!(".{cli}")).join(category);
    ...
    fn write_skill(&self, cli: &str, name: &str, desc: &str, body: &str) {
        let dir = self.tmp.path().join(cli).join(format!(".{cli}")).join("skills").join(name);
        fs::create_dir_all(&dir).unwrap();
        let content = format!("---\nname: {name}\ndescription: {desc}\n---\n\n{body}\n");
